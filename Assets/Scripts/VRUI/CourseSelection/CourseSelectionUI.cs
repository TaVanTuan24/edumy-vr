...
public class CourseSelectionUI : MonoBehaviour
{
    ...
    [SerializeField] private bool openYouTubeExternally = false;
    ...
    private async Task PlayVideoLessonAsync(LessonData lesson)
    {
        ...
        if (IsYouTubeUrl(url))
        {
            bool backendReturnedPlayableStream = !string.IsNullOrWhiteSpace(resolvedUrl) && !IsYouTubeUrl(resolvedUrl);
            bool allowExternalOpen = openYouTubeExternally && (!backendReturnedPlayableStream || autoOpenYouTubeWhenResolveFails);
            ...
            string resolverCode = ApiManager.Instance != null ? ApiManager.Instance.LastStreamResolveErrorCode : null;
            long resolverStatusCode = ApiManager.Instance != null ? ApiManager.Instance.LastResponseStatusCode : 0;
            string msg = string.IsNullOrWhiteSpace(resolverReason)
                ...
                : $"Unity could not play the YouTube video because backend stream resolution failed: {resolverReason}.";

            if (string.Equals(resolverCode, "UNAUTHORIZED", StringComparison.OrdinalIgnoreCase) || resolverStatusCode == 401)
            {
                msg += " The stream resolve API requires a valid JWT token.";
            }
            else if (!string.IsNullOrWhiteSpace(resolverReason))
            {
                msg += " Install ytdl-core/yt-dlp on the server or return a direct mp4/m3u8 URL from the resolver.";
            }
